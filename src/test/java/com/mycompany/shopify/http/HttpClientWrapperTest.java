...
/**
 * Comprehensive unit tests for HttpClientWrapper. Tests retry logic, rate limiting, error handling,
...
class HttpClientWrapperTest {
  ...
  class SuccessfulRequestTests {
    ...
    void shouldExecuteAsyncRequestSuccessfully()
      ...
      CompletableFuture<ResponseEntity<String>> future =
          httpClientWrapper.executeAsyncWithRetry(request);
  ...
  class RetryLogicTests {
    ...
    void shouldFailAfterMaxRetriesExceeded() {
      ...
      verify(restTemplate, times(3))
          .exchange(anyString(), any(), any(), eq(String.class)); // 1 initial + 3 retries
    }
  }
      ...
      when(restTemplate.exchange(anyString(), any(), any(), eq(String.class)))
          .thenThrow(
              new HttpClientErrorException(
                  HttpStatus.TOO_MANY_REQUESTS, "Rate limited", rateLimitHeaders, null, null))
    ...
    void shouldRespectMaxRetriesConfiguration() {
      ...
      HttpServerErrorException exception =
          assertThrows(
              HttpServerErrorException.class,
              () -> {
                httpClientWrapper.executeWithRetry(request, retryConfig);
              });
  ...
  class EdgeCasesTests {
    ...
    void shouldHandleVeryLongDelaysGracefully() {
      ...
      // Act
      ResponseEntity<String> result = httpClientWrapper.executeWithRetry(request, retryConfig);
